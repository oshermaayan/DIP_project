...
def run_one_init(parameters,net,NET_TYPE,net_input, imgs,OPT_OVER,OPTIMIZER, reg_noise_std, LR, num_iter,
    ...
    def closure():
        ...
        # Create batch!
        (x, y) = net_input_saved.size()[2:]
        new_net_input = net_input_saved.repeat(batch_size, 1) + (noise.normal_() * reg_noise_std).repeat(batch_size,1)
        new_net_input.size = (batch_size, 3, x, y)
        ...
        #if i%check_psnr_freq == 0:
            #max_drop_thresh = 0.05
        if parameters.psnrDropGuard:
            if i > check_psnr_freq:
                #TODO : tr  y to improve performace by using a queue instead of creating a list at each iteration
            ...
            if psnr_HR - max_psnr_in_last_iters < -2:
                ...
                psnr_history.append(psnr_history[-check_psnr_freq]) #TODO make sure this makes sense logically
                return total_loss * 0
            else:
